...
module.exports = {
    Query: {
        async getPosts() {
            ...
            try {
                await db.collection('posts').orderBy('createdAt', 'desc').limit(8).get()
                    .then(data => {
                        data.forEach(doc => {
                            posts.push({
                                id: doc.data().id,
    ...
    Mutation: {
        ...
        async nextPosts(_, { id }, context) {
            try {
                ...
                await lastPosts.get()
                    .then(doc => {
                        return db.collection('posts').orderBy("createdAt", 'desc').startAfter(doc).limit(3).get()
                            .then(data => {
                                data.forEach(doc => {
                                    posts.push({
                                        id: doc.data().id,
                    ...
                    .then(doc => {
                        ...
                        if (doc.data().owner !== username) {
                            ...
                            return commentsCollection.get()
                                ...
                                .then(data => {
                                    data.forEach(doc => {
                                        return db.collection(`/users/${doc.data().owner}/notifications`).where('postId', "==", id).get()
                                    })
                                })
                        }
                    })
        ...
        async likePost(_, { id }, context) {
            ...
            try {
                const { isLiked, likeId } = await likeCollection.where('owner', '==', username).limit(1).get()
                    .then(data => {
                            ...
                            if (!isLiked) {
                                                    ...
                                                    .then(data => {
                                                        return data.docs.forEach(doc => {
                                                            ...
                                                            if (post.owner !== username) {
                                                                return db.collection(`/users/${post.owner}/notifications`)
                                                                    .where('type', '==', "LIKE")
                                ...
                                return likeCollection.add({
                                    ...
                                    .then(data => {
                                        ...
                                        if (post.owner !== username) {
                                            db.collection(`/users/${post.owner}/notifications`).add({
        ...
        async subscribePost(_, { postId }, context) {
            const { username } = await fbAuthContext(context)
            const { name, displayImage, colorCode } = await randomGenerator(username, postId)

            let postOwner;
            let subscribe;

            try {
                await db.collection('posts').where('id', '==', postId).get()
                    .then(data => {
                        if (data.empty) {
                            throw new UserInputError("post tidak di temukan")
                        } else {
                            ...
                            return db.collection(`/posts/${postId}/subscribes`).add(subscribe)
                                .then(data => {
                                    data.update({ id: data.id })

                                    if (postOwner !== username) {
                                        db.collection(`/users/${postOwner}/notifications`).add({
                                            recipient: postOwner,
                                            sender: username,
                                            read: false,
                                            postId: postId,
